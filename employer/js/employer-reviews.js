document.addEventListener("DOMContentLoaded", async () => {
  ...
  container.innerHTML = ""; // تفريغ الكروت القديمة
        ...
        <div class="col-md-6 col-lg-4">
          <div class="card border-0 shadow rounded-4 p-3">
            <div class="d-flex align-items-center mb-2">
              <img src="https://i.pravatar.cc/50?u=${rating.workerId}" class="rounded-circle me-3" alt="avatar">
              <div>
                <h5 class="mb-0">${rating.workerName || "عامل مجهول"}</h5>
                <small class="text-muted">${rating.workerJobTitle || ""}</small>
              </div>
            </div>
            <div class="mb-2 text-end">${starIcons}</div>
            <p class="bg-light rounded p-3 text-black-50">${rating.comment || "بدون تعليق"}</p>
            <div class="text-end text-muted small">${rating.date?.split("T")[0] || ""}</div>
          </div>
        </div>
      `;
