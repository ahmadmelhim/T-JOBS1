document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    const res = await fetch("http://tjob.tryasp.net/api/Worker/Rates/MyRatings", {
    ...
    ratings.forEach(rating => {
      ...
      const imgSrc = rating.employerImg || "https://i.pravatar.cc/50?u=" + rating.employerName;
        ...
        <div class="col-md-6 col-lg-4">
          <div class="card border-0 shadow rounded-4 p-3">
            <div class="d-flex align-items-center mb-2">
              ...
              <div>
                <h5 class="mb-0">${rating.employerName || "صاحب عمل مجهول"}</h5>
                <small class="text-muted">${rating.jobTitle || ""}</small>
              </div>
            </div>
            <div class="mb-2 text-end">${starIcons}</div>
            <p class="bg-light rounded p-3 text-black-50">${rating.note || "بدون تعليق"}</p>
            <div class="text-end text-muted small">${rating.ratedAt?.split("T")[0] || ""}</div>
          </div>
        </div>
      `;
